export const APP_NAME = 'masspow'
export const MAIN_TITLE = `${APP_NAME} | Supplements, gym and sport articles`
export const DESCRIPTION = 'Grow your body massively with our nutritional supplements! You can get your training equipment here too.'
export const LAYOUT_FOOTER_ATTRIBUTION = 'Designed and developed by'
export const HOME_PAGE_STRINGS = {
  mainTitle: `Welcome to ${APP_NAME}!`,
  mainDescription: 'Get advised by experts in your diet and training plannings.',
  callToAction: 'View our combos',
  secondaryAction: 'Explore products'
}
export const PRODUCTS_PAGE_STRINGS = {
  mainTitle: 'Catalog'
}
export const PRODUCT_PREVIEW_COMPONENT_STRINGS = {
  priceText: 'Price',
  stockText: 'Stock',
  addToCartBtn: 'Add to cart',
  removeFromCartBtn: 'Remove from cart',
  qtyInputLabel: 'Quantity',
  detailsBtn: 'View details'
}
export const CART_MODAL_STRINGS = {
  title: 'Your shopping cart',
  emptyCartMessage: 'You did not add items yet.',
  tableHeaders: {
    name: 'Name',
    brand: 'Brand',
    quantity: 'Quantity',
    price: 'Price',
    totalPrice: 'Subtotal',
    image: 'Image'
  },
  buyBtn: 'Finish buying',
  closeBtn: 'Close'
}
export const FILTERS_MODAL_STRINGS = {
  title: 'Which products are you looking for?'
}
